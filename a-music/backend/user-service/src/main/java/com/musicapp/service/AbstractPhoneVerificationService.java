...
public abstract class AbstractPhoneVerificationService implements PhoneVerificationService {
    ...
    public boolean isVerified(String phone) {
        String token = TokenContextHolder.getVerificationToken(phone);
        ...
        boolean isSamePhones = phone.equals(verifiedPhone);

        if (isSamePhones) {
            TokenContextHolder.removeVerificationToken(phone);
        }
